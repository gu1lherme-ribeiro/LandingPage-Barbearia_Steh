document.addEventListener("DOMContentLoaded", () => {
  ...
  function preloadElements() {
    const sections = document.querySelectorAll(
      ".header, .hero, .services, .about, .testimonials, .booking, .contact, .footer"
    );
      ...
      () => {
        ...
        const sections = [
          ...
          ...document.querySelectorAll(
            ".services, .about, .testimonials, .booking, .contact, .footer"
          ),
        ].filter(Boolean);
        ...
        sections.forEach((section, index) => {
          setTimeout(
            ...
            isMobile ? 30 * index : 50 * index
          ); // Tempo reduzido em dispositivos móveis
        });
      },
      isMobile ? 500 : 800
    ); // Tempo reduzido em dispositivos móveis
